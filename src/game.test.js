...
describe("test gameboard", () => {
    test("correctly places ship", () => {
        ...
        expect(g.pointToRootPart[0][0]).toStrictEqual([0, 0]);
        expect(g.pointToRootPart[0][2]).toStrictEqual([0, 0]);
        expect(g.pointToRootPart[0][3]).toStrictEqual([0, 0]);
        expect(g.pointToRootPart[0][4]).toBeUndefined();
    });
    ...
    test("receive attack correctly", () => {
        ...
        g.placeShip([0, 0], "south", 4);
        g.receiveAttack([0, 0]);
        // expect(g.misses.length).toBe(0);
        expect(g.hits[0][0]).toBeTruthy();
        expect(g.ships[0][0].hitCount).toBe(1);
    });
